...
Sprites::Sprites(SDL_Window* gWindow,SDL_Renderer* gRenderer,string Path,int NoS,int posX,int posY,int sWidth, int sHeight,int width, int height,string description,bool colorKey,int df):position(posX,posY),Size(width,height)
{
    ...
    SDL_Surface* SD_Surface = IMG_Load(Path.c_str());
    if (colorKey)
...
Sprites::Sprites(SDL_Window* gWindow,SDL_Renderer* gRenderer,string Path,int NoS,int sWidth, int sHeight,SDL_Rect* externalDestination,string description,bool colorKey,int df):position(externalDestination->x,externalDestination->y),Size(externalDestination->w,externalDestination->h)
{
    this->gWindow = gWindow;
    this->gRenderer = gRenderer;
    this->NOS = NoS;
