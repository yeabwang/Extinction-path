...
Sprites::Sprites(SDL_Window* gWindow, SDL_Renderer* gRenderer, const char* Path, int NoS, int posX, int posY, int sWidth, int sHeight, int width, int height, const char* description, bool colorKey, int df)
    ...
    this->NOS = NoS;
    this->sRect = new SDL_Rect*[NoS]{ nullptr };
    ...
    if (!SD_Surface) {
        ...
        Texture = nullptr; // Ensure Texture is null on failure
        return;
    }
    ...
    if (Texture) { // Only allocate sRect if Texture loaded
        for (int i = 0; i < NOS; i++) {
...
void Sprites::render(int Frame, SDL_RendererFlip flip, double angle) {
    if (!isValid()) return; // Skip rendering if invalid
    if (Frame) {
        if (!(Frame % NOS)) cs += 1;
    } else {
        cs += 1;
    ...
    if (cs >= NOS) cs = 0;
    SDL_RenderCopyEx(gRenderer, Texture, sRect[cs], dRect, angle, nullptr, flip);
}

void Sprites::setAngle(double a) {
    angle = a;
}
...
Point Sprites::get_Position() {
    return position; // Already returns Point(dRect->x, dRect->y)
}

Point Sprites::get_Size() {
    return Size; // Already returns Point(dRect->w, dRect->h)
}
