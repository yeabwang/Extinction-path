...
void Hero::EventsController(SDL_Event* e) {
    ...
    static int fireCooldown = 0; // Added firing cooldown
    if (counter != 0) {
        counter--;
    }
    if (fireCooldown > 0) {
    ...
    if (e->key.keysym.sym == SDLK_SPACE && e->type == SDL_KEYUP && e->key.repeat == 0 && fireCooldown == 0) {
        count = 20;
        fire = true;
        if (shootUp) {
            if (!flip)
        ...
        sound[0]->Play();
        fireCooldown = 1; // ~0.16s cooldown at 60 FPS
    }
}
...
void Hero::CollisionImpact(GameObjects* CollidedWith) {
    if (!CollidedWith) return; // Null check to prevent crashes
...
void LoadHeroFromFile(FILE* file, Hero* hero) {
    ...
    fscanf(file, "%*s");
    fscanf(file, "%d", &temp);
    hero->flip = (SDL_RendererFlip)temp;
    fscanf(file, "%d", &temp);
    hero->moved = (temp != 0);
    fscanf(file, "%d", &temp);
    hero->Alive = (temp != 0);
    fscanf(file, "%d", &temp);
    hero->collided = (temp != 0);
